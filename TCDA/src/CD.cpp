...
/*
  ...
  for (i=0; i<33; i++)
    for (j=0; j<33; j++) 
      if (Rfilter_[i][j].size()) printf("%d, %d\n",i,j);
...
/*
	...
	((spr >= p->srcPortright_) && (spl <= p->srcPortleft_)) )
 	...
 	if( 
     	((!protocol || !p->pro_num_ ) || (protocol == p->pro_num_))
	&& ( !( (dpr< p->destPortleft_ ) || (dpl > p->destPortright_ ) ) )
	&& ( !( (spr < p->srcPortleft_ ) || (spl > p->srcPortright_ ) ) )
      	){
		if (dIP_len >= p->destIPLen_ ){
        		if(
			( (dpl <= p->destPortleft_) && (dpr >= p->destPortright_) ) &&
			( (spl  <= p->srcPortleft_)  && (spr  >= p->srcPortright_) ) &&
			( (protocol == p->pro_num_) || (protocol == 0) ) 
			)
        		;
			else
			Conflictnum++;
...
exact++;
	...
	&& ( !( (fp->destPortright_ < p->destPortleft_ ) || (fp->destPortleft_ > p->destPortright_ ) ) )
	&& ( !( (fp->srcPortright_ < p->srcPortleft_ ) || (fp->srcPortleft_ > p->srcPortright_ ) ) )
      	){ 
		//if (fp->srcIPLen_ > p->srcIPLen_ ){     
			if (fp->destIPLen_ >= p->destIPLen_ ){
	        		if(
				( (fp->destPortleft_ <= p->destPortleft_) && (fp->destPortright_ >= p->destPortright_) ) &&
				( (fp->srcPortleft_  <= p->srcPortleft_)  && (fp->srcPortright_  >= p->srcPortright_) ) &&
				( (fp->pro_num_ == p->pro_num_) || (fp->pro_num_ == 0) ) 
				)
	        		;
				else
				Conflictnum++;
